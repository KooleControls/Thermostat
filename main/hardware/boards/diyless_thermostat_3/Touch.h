...
// ──────────────────────────────────────────────────────────────
// GT911 capacitive touch (I2C) for the DIYLESS Thermostat 3.
// Shares the I2C bus (SDA17/SCL18) with the AHT20 sensor. There is no
// ESP-controlled touch reset line; the controller is already running from
// power-on, so we just auto-probe its address (0x5D default, 0x14 backup —
// this unit reports 0x14) and read by polling (esp_lvgl_port). INT is wired to
// GPIO10 but left unused (NC) to match the proven polling pattern.
//
...
public:
    bool Init()
    {
        ...
        // Build the GT911 panel-IO config explicitly (the library macro's C
        // designated-initializer order doesn't match the struct in C++).
        ...
        tp_cfg.rst_gpio_num = GPIO_NUM_NC;  // no ESP-controlled touch reset
        tp_cfg.int_gpio_num = GPIO_NUM_NC;  // INT (GPIO10) unused — we poll
