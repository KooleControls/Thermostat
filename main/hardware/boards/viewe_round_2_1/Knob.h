...
// by the PCNT peripheral, plus a push-button on a direct GPIO (active-low; the
// board has an external pull-up). Exposed to LVGL as an ENCODER indev.
...
private:
    ...
    void Read(lv_indev_data_t *data)
    {
        ...
        bool pressed = gpio_get_level((gpio_num_t)BoardConfig::ENCODER_PIN_SW) == 0;  // active-low
