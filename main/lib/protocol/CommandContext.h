...
enum class ArgType : uint8_t { String, UInt32, Float, Bool };
...
inline ArgSpec Required(const char* name, float& dst)     { return { name, &dst, 0, ArgType::Float,  true }; }
...
inline ArgSpec Optional(const char* name, float& dst)     { return { name, &dst, 0, ArgType::Float,  false }; }
